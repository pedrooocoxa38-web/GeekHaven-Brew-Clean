...
export const authAPI = {
  async login(email: string, password: string): Promise<LoginResponse> {
    // Se está em modo debug e as credenciais são do admin, simula sucesso
    if (import.meta.env.VITE_DEBUG_MODE === 'true' && 
        email === 'admin@geekhaven.com' && 
        password === 'admin123') {
      console.log('🔧 Debug Mode: Simulando login do admin');
    ...
    return apiRequest<LoginResponse>('/auth/login', {
      ...
      // Se deu erro 500 e estamos em debug mode, sugere usar credenciais mock
      if (error.status === 500 && import.meta.env.VITE_DEBUG_MODE === 'true') {
        throw new APIError(500, 'Backend com problemas. Tente usar: admin@geekhaven.com / admin123');
      }
      throw error;
    });
  },
